...
import { IPlanet } from '@/types/appTypes';


export default function People() {
  const [items, setItems] = useState<IPlanet[]>([]);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [totalPages, setTotalPages] = useState<number>(1);

  useEffect(() => {
    const fetchItems = async () => {
      const { documents, totalPages } = await fetchData(`/planets?page=${currentPage}&pageSize=10`);
      setItems(documents);
